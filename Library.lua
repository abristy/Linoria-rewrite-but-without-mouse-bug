...
function Library:CreateWindow(WindowTitle)
    ...
    if Input.KeyCode == Enum.KeyCode.RightControl or (Input.KeyCode == Enum.KeyCode.RightShift and (not Processed)) then
        ...
        while Outer.Visible do
            ...
            local viewportPos, isVisible = workspace.CurrentCamera:WorldToViewportPoint(Mouse.Hit.p)
            if isVisible then
                mousePos = Vector2.new(viewportPos.X, viewportPos.Y)
            end

            Cursor.Color = Library.AccentColor;
            Cursor.PointA = mousePos;
            Cursor.PointB = mousePos + Vector2.new(6, 14);
            Cursor.PointC = mousePos + Vector2.new(-6, 14);
